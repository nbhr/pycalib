...
class TestPyCalibDistortion(unittest.TestCase):
    ...
    def test_distortion(self):
        ...
        for x, K, D, R, T, P in zip(X_2d, K_all, D_all, R_all, T_all, P_all):
            ...
            #u = cv2.undistortPointsIter(x, K, D, np.eye(3), K, (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 1e-8))
            ...
            np.testing.assert_allclose(x, d, rtol=1e-3, atol=1e-6)
            U.append(u)
        U = np.array(U).reshape((Nc, -1, 2))

        # triangulate
        X = pycalib.calib.triangulate_Npts(U, P_all)
        np.testing.assert_allclose(X[:3], X_3d[:3], atol=1e-6)
